...
from src.metrics.bootstrap import bootstrap_metric_cis
from src.metrics.distribution_metrics import summarize_distribution_metrics
from src.metrics.entropy_metrics import entropy_bits_tensor, entropy_correlations, entropy_error_metrics
...
def compute_report_metrics(predicted: torch.Tensor, target: torch.Tensor, seed: int = 42) -> dict[str, float]:
    metrics = summarize_distribution_metrics(predicted, target)
    metrics.update(entropy_correlations(predicted, target))
    metrics.update(entropy_error_metrics(predicted, target))
    ...
    metrics.update(bootstrap_metric_cis(predicted, target, seed=seed))
    return metrics


def _build_model_and_loader(config: dict, checkpoint_path: str | Path, split: str):
    ...
    model.to(device)
    return model, split_to_loader[split], device


def evaluate_checkpoint(config: dict, checkpoint_path: str | Path, split: str = "test") -> dict[str, float]:
    model, loader, device = _build_model_and_loader(config, checkpoint_path, split)
    started = time.time()
    predicted, target = collect_predictions(model, loader, device)
    metrics = compute_report_metrics(predicted, target, seed=int(config.get("seed", 42)))
